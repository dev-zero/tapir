...
build_thermal_sans_mono() {
    ...
    local bdf24="$BUILD_DIR/thermal-sans-mono/thermal-sans-mono-24/thermal-sans-mono-24.bdf"
    local out24="$FONTS_DIR/ThermalSansMono-24.ttf"
    python3 "$SCRIPT_DIR/bdf2ttf.py" "$bdf24" "$out24" "Thermal Sans Mono 24"

    local bdf17="$BUILD_DIR/thermal-sans-mono/thermal-sans-mono-17/thermal-sans-mono-17.bdf"
    local out17="$FONTS_DIR/ThermalSansMono-17.ttf"
    python3 "$SCRIPT_DIR/bdf2ttf.py" "$bdf17" "$out17" "Thermal Sans Mono 17"

    local ok=0
    [[ -f "$out24" ]] && { echo "  ✓ ThermalSansMono-24.ttf"; ok=$((ok+1)); }
    [[ -f "$out17" ]] && { echo "  ✓ ThermalSansMono-17.ttf"; ok=$((ok+1)); }
    [[ $ok -eq 2 ]] || { echo "  ✗ Failed to generate Thermal Sans Mono TTFs"; return 1; }
}
