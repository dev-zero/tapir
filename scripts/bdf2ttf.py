...
def parse_bdf_pixel_size(path):
    """Extract PIXEL_SIZE from BDF global properties."""
    with open(path, "r") as f:
        for line in f:
...
def build_ttf(bdf_path, ttf_path, family_name=None):
    EM = 1000
    PIXEL_SIZE = parse_bdf_pixel_size(bdf_path) or 24
    SCALE = EM / PIXEL_SIZE
    ASCENT = 800
    DESCENT = 200
    ...
    fb.setupNameTable({
        ...
        "styleName": "Regular",
        "uniqueFontIdentifier": f"{family_name.replace(' ', '')}-Regular",
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(f"Usage: {sys.argv[0]} INPUT.bdf OUTPUT.ttf [FAMILY_NAME]", file=sys.stderr)
        sys.exit(1)
    family = sys.argv[3] if len(sys.argv) > 3 else None
    build_ttf(sys.argv[1], sys.argv[2], family)
